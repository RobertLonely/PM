...
function add_category () {
  ...
  $rows = my_execute("insert into categories values (null, '{$slug}', '{$name}');");
...
function edit_category () {
  ...
  $rows = my_execute("update categories set slug = '{$slug}', name = '{$name}' where id = {$id}");

  $success= $rows > 0;
  $message= $rows <= 0 ? '更新失败！' : '更新成功！';
}
...
if (empty($_GET['id'])) {
  ...
  //客户端通过URL传递了一个ID,是要来拿一个修改数据的表单,所以需要拿到用户想要修改的数据
  $current_edit_category = my_fetch_one('select * from categories where id = ' . $_GET['id']);
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    edit_category();
  }
}
        ...
        <div class="col-md-4">
          ...
          <form action="<?php echo $_SERVER['PHP_SELF']; ?>?id=<?php echo $current_edit_category['id']; ?>" method="post">
            ...
            <div class="form-group">
              ...
              <p class="help-block">https://Lee.me/category/<strong>slug</strong></p>
            </div>
            <div class="form-group">
