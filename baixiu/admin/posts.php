...
$total_count = (int)my_fetch_one("select count(1) as count from posts
...
$begin = $begin < 1 ? 1 : $begin; // 确保begin不会小于 1
...
// 处理数据格式转换================================================================
function convert_status ($status) {// 转换状态显示
  $dict = array(
    'published' => '已发布',
    'drafted' => '草稿',
    'trashed' => '回收站'
...
function convert_date ($created) {//转换时间格式
  // 如果配置文件没有配置时区,date_default_timezone_set('PRC');
    ...
    <div class="container-fluid">
      ...
      <div class="page-action">
        ...
        <ul class="pagination pagination-sm pull-right">
         <li><a href="?page=<?php echo $page-$visiables ?>">上一页</a></li>
           <?php for ($i = $begin; $i <= $end; $i++): ?>
          <li<?php echo $i === $page ? ' class="active"' : '' ?>><a href="?page=<?php echo $i . $search; ?>"><?php echo $i; ?></a></li>
          <?php endfor ?>
          <li><a href="?page=<?php echo $page+$visiables ?>">下一页</a></li>
        </ul>
      </div>
